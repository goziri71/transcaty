...
Issuers (shown in authenticator apps): `PORTAL_MFA_ISSUER`, `PROVIDER_MFA_ISSUER` (optional; default from `EMAIL_APP_NAME` or `"Transcaty Portal"` / `"Transcaty Provider"`).

**DB migration:** `npm run db:migrate` (adds `mfa_*` columns on `merchant_users` and `provider_users`).

## Metrics (APM-style)

- **`GET /metrics`** — Prometheus text format (`transcaty_*` metrics + Node.js defaults).
...
- **HTTP metrics:** `transcaty_http_requests_total`, `transcaty_http_request_duration_seconds` (labels: `method`, `route`, `status_code`). `/metrics` and `/health` are excluded from these histograms.
