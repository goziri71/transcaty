...
| **2** | **Get** transaction (optional) | `GET /v1/transactions/:transactionId` | Use `transactionId` from step 1. Expect `status: pending`, `rail: india`, `railLabel: India UPI (H2H)`, `currency: USDT` (settlement pocket; `currencySymbol: INR` on create means **payer pays INR**, wallet credits **USDT**). |
