/**
 * Brazil payout flow (PIX): account inquiry, create payout, handle PayOK callback.
 *
 * Concurrency model:
...
import { payokPayoutAccountInquiry, payokPayoutCreate } from "../bangladesh/provider/client.js";
...
// Reuse the Bangladesh PayoutCreationError so the merchant-facing error mapper
// (which duck-types on err.name === "PayoutCreationError") stays consistent.
import { PayoutCreationError } from "../bangladesh/payout.js";
...
export async function createPayoutOrder(params: {
  ...
  const created = await db.transaction(async (txDb) => {
    ...
    if (!wallet) {
      throw new Error("Merchant BRL wallet not found");
    }
